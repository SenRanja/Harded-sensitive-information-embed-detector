	...
	for gitdiffFile := range gitdiffFiles {
		...
		s.Go(func() error {
			for _, textFragment := range gitdiffFile.TextFragments {
				if textFragment == nil || len(textFragment.Lines) > 12000 {
					return nil
				}
	...
	s.Go(func() error {
		...
		return filepath.Walk(source,
			func(path string, fInfo os.FileInfo, err error) error {
				...
				//我在这里进行了修改
				//下面可以匹配不想进行匹配的文件名的正则表达式
				TargetFileNameExp := regexp.MustCompile(`\.gitignore`)
				//下面进行文件名正则匹配和文件大小的选择性不检测，避免卡死
				if fInfo.Mode().IsRegular() && !TargetFileNameExp.MatchString(fInfo.Name()) && fInfo.Size() < 2*1024*1024 {
					paths <- path
				}
				return nil
			})
